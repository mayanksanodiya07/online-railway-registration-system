...
public class BookingServiceImpl implements BookingService {
	...
	@Override
	public Mono<Booking> createBooking(Long trainId, int userId, int seatsBooked) {
		return Mono.fromCallable(() -> {
			logger.info("Creating booking for trainId: {}, userId: {}, seats: {}", trainId, userId,
					seatsBooked);
			...
			try {
				// Get train information using Feign client
				...
				if (trainResponse.getStatusCode() == HttpStatus.NOT_FOUND) {
					...
					throw new ResponseStatusException(HttpStatus.NOT_FOUND,
							"Train not available with this ID, please enter valid train ID");
				}
				...
				Booking booking = Booking.builder().trainId(trainId).userId(userId)
						.seatsBooked(seatsBooked).bookingTime(LocalDateTime.now()).build();
	...
	@Override
	public Mono<List<Booking>> getBookingsByTrainId(Long trainId) {
		return Mono.fromCallable(() -> bookingRepository.findByTrainId(trainId))
	...
	public Mono<List<Booking>> getAllBookings() {
		return Mono.fromCallable(() -> bookingRepository.findAll()).subscribeOn(Schedulers.boundedElastic())
				.onErrorMap(e -> {
